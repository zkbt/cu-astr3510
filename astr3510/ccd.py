...
class Camera:
    def __init__(self, shape=(30, 50),
        ...
        self.initialize_photons(star_positions=star_positions, star_brightnesses=star_brightnesses, sky_brightness=sky_brightness, pixel_scale=pixel_scale, psf=psf)


    def initialize_photons(self, star_positions=[[12, 20]],
                            star_brightnesses=[100]*u.electron/u.s,
                            pixel_scale=0.7*u.arcsec/u.pixel,
                            sky_brightness=3*u.electron/u.s,
                            psf=seeing(fwhm=2.0*u.arcsec)):
        ...
        for i in range(N):
            ...
            theta = np.sqrt((x - x_center)**2 + (y-y_center)**2)*u.pixel*self.pixel_scale
    ...
    def initialize_dark(self):
        self.dark_rate = np.random.exponential(self.average_dark_rate.to_value(u.electron/u.s), self.shape)*u.electron/u.s
    ...
    def make_light_model(self, exposure_time=1*u.s):
        return self.photon_rate/self.gain*exposure_time + self.dark_rate*exposure_time/self.gain + self.make_bias_model()
    ...
    def make_Elight_noise_model(self, exposure_time=1*u.s):
        return np.sqrt(self.make_bias_noise_model()**2 + self.make_dark_noise_model(exposure_time)**2 + self.make_light_noise_model(exposure_time)**2)

    def make_bias_noise_instance(self, *args, **kwargs):
        return np.random.normal(0, self.make_bias_noise_model(*args, **kwargs).to_value(u.adu))*u.adu

    def make_dark_noise_instance(self, *args, **kwargs):
        return np.random.normal(0, self.make_dark_noise_model(*args, **kwargs).to_value(u.adu))*u.adu

    def make_light_noise_instance(self, *args, **kwargs):
        return np.random.normal(0, self.make_light_noise_model(*args, **kwargs).to_value(u.adu))*u.adu
    ...
    def imshow_exposures(self, exposure_time=1*u.s, include_noise=False, vmin=None, vmax=None):

        b, d, l = self.create_images(exposure_time=exposure_time, include_noise=include_noise)
        if vmin is None:
            vmin=np.min([b,d,l])
        if vmax is None:
            vmax=np.max([b,d,l])

        fi, ax = plt.subplots(1,4, figsize=(8, 3), dpi=600, sharex=True, sharey=True, constrained_layout=True)
        for i, x, t in zip(range(3),
                           [b,d,l],
                           ['bias', 'dark + bias', 'light + dark + bias']):
            imshowed = ax[i].imshow(x, vmin=vmin, vmax=vmax, interpolation='nearest')
            ax[i].set_title(t)
            plt.colorbar(imshowed, orientation='horizontal', ax=ax[i], label='Pixel Brightness (ADU)')
        for a in ax:
            a.set_xticks(np.arange(-.5, self.shape[1], 1))
            a.set_yticks(np.arange(-.5, self.shape[0], 1))
            a.set_xticklabels([])
            a.set_yticklabels([])
...
{noise_string}'''
    ...
    def plot_pixels_with_exposure_time(self, exposure_times=np.linspace(0, 10)*u.s, include_noise=False, vmin=None, vmax=None):
        ...
        for i, e in enumerate(exposure_times):
            b_cube[i,:,:], d_cube[i,:,:], l_cube[i,:,:] = self.create_images(exposure_time = e, include_noise=include_noise)

        for title in ['bias', 'bias + dark', 'bias + dark + light']:
            fi, ax = plt.subplots(self.shape[0], self.shape[1], constrained_layout=True, dpi=600, sharex=True, sharey=True, figsize=(8,6))
            for i in range(rows):
                for j in range(cols):
                    ax[i,j].plot(exposure_times, b_cube[:,i,j], label='bias', linewidth=2, alpha=0.5, linestyle=':')
                    plt.ylim(0.95*np.min(b_cube), 1.05*np.max(b_cube))
                    if 'dark' in title:
                        ax[i,j].plot(exposure_times, d_cube[:,i,j], label='dark', linewidth=2, alpha=0.5, linestyle='--')
                        plt.ylim(0.95*np.min(b_cube), 1.05*np.max(d_cube))
                    if 'light' in title:
                        ax[i,j].plot(exposure_times, l_cube[:,i,j], label='light', linewidth=2, alpha=0.5, linestyle='-')
