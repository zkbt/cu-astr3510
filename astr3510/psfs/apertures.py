...
class Aperture:
    ...
    def display_psf(
            self,
            theta_range=10 * u.arcsec,
            stretches=['linear', 'log'],
        ):
            ...
            with quantity_support():
                ...
                for i, scale in enumerate(stretches):
                    ...
                    plt.sca(ax[0, i])
                    self.plot_psf(theta_range=theta_range, scale=scale)
                    plt.xlabel('')

                    plt.sca(ax[1, i])
                    self.imshow_psf(scale=scale)
                    if scale == "log":
    ...
    def imshow_aperture(self, **kw):
        with quantity_support():

            imshow_kw = dict(cmap='gray', extent=[min(self.x), 
                            max(self.x), 
                            min(self.y), 
                            max(self.y)])
            imshow_kw.update(**kw)
            plt.imshow(self.aperture, **imshow_kw)
            plt.xlabel(r'$\sf x$ at Telescope Aperture' + f' ({self.x.unit.to_string("latex_inline")})')
            plt.ylabel(r'$\sf y$ at Telescope Aperture' + f' ({self.y.unit.to_string("latex_inline")})')


    def imshow_psf(self, scale='log', **kw):
        with quantity_support():
            if scale == "log":
                ...
                vmin = 0
                vmax = 1
            imshow_kw = dict(cmap='gray', extent=[min(self.theta_x.value), 
            ...
            plt.xlabel(r'$\sf \theta_x$ at Focal Plane' + f' ({self.theta_x.unit.to_string("latex_inline")})')
            plt.ylabel(r'$\sf \theta_y$ at Focal Plane' + f' ({self.theta_y.unit.to_string("latex_inline")})')

    def plot_psf(self, theta_range=10*u.arcsec, scale='log'):
        with quantity_support():
            ...
            plt.yscale(scale)
            plt.ylim(ymin, None)
            plt.xlabel(r'$\sf \theta$ at Focal Plane' + f' ({self.theta_x.unit.to_string("latex_inline")})')
            plt.ylabel('Relative Intensity')


example_apertures_directory = os.path.join(astr3510_directory, 'psfs', 'apertures')
default_aperture_image_filename =  os.path.join(example_apertures_directory, 'leavitt.jpg')

class BitmapAperture(Aperture):
    def make_aperture(self, filename=default_aperture_image_filename, diameter=0.1*u.m, **kw):
...
class JWST(BitmapAperture):
    def make_aperture(self, **kw):
        BitmapAperture.make_aperture(self, 
                                     filename=os.path.join(example_apertures_directory, 'jwst.jpg'), 
                                     diameter=6.5*u.m, 
                                     **kw)
        """    def make_aperture(self, **kw):
            image = np.sum(plt.imread('jwst-selfie.jpg'), -1)[120:1060,:]
            image = image / np.nanmax(image)
            #image = image > 0.5
            rows, columns = np.shape(image)
...
class ShapeAperture(Aperture):
    def make_aperture(self, radius=1*u.m, N=101, buffer=1.1, **kw):
...
class RectangleAperture(ShapeAperture):
    def make_aperture(self, width=1*u.m, height=1*u.m, **kw):
        ...
        self.aperture[(np.abs(self.x2d) < width/2)*(np.abs(self.y2d) < height/2)] = 1
...
class ObscuredCircleAperture(CircleAperture):
    def make_aperture(self, diameter=1*u.m, obscuration_diameter=0.5*u.m, spiders=4, spider_width=5*u.cm, **kw):
...
class Spitzer(ObscuredCircleAperture):
    def make_aperture(self, **kw):
        ObscuredCircleAperture.make_aperture(self, 
                                     diameter=0.85*u.m, 
...
class Hubble(ObscuredCircleAperture):
    def make_aperture(self, **kw):
        ...
        for blob_theta in np.array([30, 150, 270])*np.pi/180:
            self.subtract_circle(diameter=blob_diameter, 
...
apertures_to_test = [CircleAperture, ObscuredCircleAperture, RectangleAperture, JWST, BahtinovAperture, Hubble, Spitzer, Leto]
